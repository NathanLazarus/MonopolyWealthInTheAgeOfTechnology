...
job_list = list(itertools.product(seed_specs, chi_specs, policy_specs))

[seed, chi, gov] = job_list[args.job_ID]
...
filename = 'SurvivalOutcomes_seed_' + str(seed) + '_chi_' + str(chi)  + '_policy_' + str(gov) + '.csv'

utilityarray = np.zeros((Nperiods, len(spec_list)))
...
PDVutility = np.zeros((Nplayers, len(spec_list)))
...
for counter, specification in enumerate(spec_list):
    ...
    present_discounted_utility = sum1(aggregatearray[:, 0] * beta ** np.arange(Nperiods))
    ...
    PDVutility[:, counter] = present_discounted_utility

np.savetxt(filename, np.hstack((endingMarketShares, endingRDShares, secondToLastPeriodRDShares, PDVutility)), delimiter=',', header=','.join(investment_spec_strings * 4), comments='')
